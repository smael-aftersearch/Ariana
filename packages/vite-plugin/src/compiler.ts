...
function appendAnimationHelperLines(lines: string[]) {
  ...
  lines.push(`  const __ari_timeMs = (value) => String(value ?? '').split(',').reduce((max, part) => { const text = part.trim(); const amount = Number.parseFloat(text) || 0; const ms = text.endsWith('ms') ? amount : text.endsWith('s') ? amount * 1000 : amount; return Math.max(max, ms); }, 0);`);
  lines.push(`  const __ari_motionMs = (element) => { const style = getComputedStyle(element); return Math.max(__ari_timeMs(style.animationDuration) + __ari_timeMs(style.animationDelay), __ari_timeMs(style.transitionDuration) + __ari_timeMs(style.transitionDelay)); };`);
  ...
  lines.push(`  const __ari_removeNodes = (nodes) => { const nodeList = Array.from(nodes); const targets = __ari_collectAnimated(nodeList, 'data-ari-animate-leave'); const removeAll = () => { for (const node of nodeList) node.parentNode?.removeChild(node); }; if (targets.length === 0) { removeAll(); return; } let pending = targets.length; let finished = false; let timeout = 0; const finish = () => { if (finished) return; finished = true; clearTimeout(timeout); removeAll(); }; let fallbackMs = 0; for (const element of targets) { const classes = __ari_animationClasses(element.getAttribute('data-ari-animate-leave')); element.removeAttribute('data-ari-animate-leave'); let settled = false; const done = () => { if (settled || finished) return; settled = true; pending--; if (pending === 0) finish(); }; element.addEventListener('animationend', done, { once: true }); element.addEventListener('transitionend', done, { once: true }); if (classes.length) { element.classList.add(...classes); fallbackMs = Math.max(fallbackMs, __ari_motionMs(element)); if (fallbackMs === 0) done(); } else done(); } if (!finished) timeout = setTimeout(finish, Math.min(Math.max(fallbackMs + 80, 120), 5000)); };`);
}
