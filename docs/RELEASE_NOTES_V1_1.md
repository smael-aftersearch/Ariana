...
Socket flagged `@ariana-framework/core@1.0.0` for dynamic code execution. The published tarball contained runtime template files using the JavaScript Function constructor, which is correctly classified as eval-like behavior.
...
- Added tarball inspection for dynamic-code and unsafe DOM-write patterns.
- Added `npm run security:audit`.
- Added static repository scan for common secret/token patterns.
- Added unsafe API checks for dynamic-code execution, unsafe DOM writes, and non-approved child-process execution.
